...
-- NGR specific, StatusValues is empty in NGR 2. Be safe, inserting only if not in the database.
INSERT INTO StatusValues (id, name, reserved, displayorder)
       SELECT 0, 'unknown', 'y', 0 WHERE NOT EXISTS (SELECT 1 FROM StatusValues WHERE id=0);

INSERT INTO StatusValues (id, name, reserved, displayorder)
       SELECT 1, 'draft', 'y', 1 WHERE NOT EXISTS (SELECT 1 FROM StatusValues WHERE id=1);

INSERT INTO StatusValues (id, name, reserved, displayorder)
       SELECT 2, 'approved', 'y', 3 WHERE NOT EXISTS (SELECT 1 FROM StatusValues WHERE id=2);

INSERT INTO StatusValues (id, name, reserved, displayorder)
       SELECT 3, 'retired', 'y', 5 WHERE NOT EXISTS (SELECT 1 FROM StatusValues WHERE id=3);

INSERT INTO StatusValues (id, name, reserved, displayorder)
       SELECT 4, 'submitted', 'y', 2 WHERE NOT EXISTS (SELECT 1 FROM StatusValues WHERE id=4);

INSERT INTO StatusValues (id, name, reserved, displayorder)
       SELECT 5, 'rejected', 'y', 4 WHERE NOT EXISTS (SELECT 1 FROM StatusValues WHERE id=5);
